...
**Factories.ts** is a lightweight library which provides a domain-specific language (DSL) for coding markup directly in TypeScript to generate HTML, but also SVG and MathML on the server side. The advantage is obvious: Instead of a separate template language with its own engine, the HTML structure can be built dynamically using ordinary JavaScript functions:
...
import { html, head, title, body, p } from "factories"
